...
if path.stem == 'TCGA-ML':
    cwd = path
else:
    cwd = list(path.parents)[::-1][path.parts.index('TCGA-ML')]
    import sys
    sys.path.append(str(cwd))


tcga_maf = pickle.load(open('/home/janaya2/Desktop/ATGC2/files/tcga_maf_table.pkl', 'rb'))
tcga_maf = tcga_maf.loc[tcga_maf['CDS'] > 0]
samples = pickle.load(open('/home/janaya2/Desktop/ATGC2/files/tcga_sample_table.pkl', 'rb'))
...
samples = samples.loc[samples['Tumor_Sample_Barcode'].isin(tcga_maf['Tumor_Sample_Barcode'].unique())]
...
with open(cwd / 'data' / 'data.pkl', 'wb') as f:
